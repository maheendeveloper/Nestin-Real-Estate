    ...
    <div className='lg:hidden w-full bg-white border-t shadow-sm'>
      {navItems.map((item)=>(
        <div key={item.name} className="border-b border-gray-200 px-4 py-3">
          <button
          onClick={()=>toggledropdown(item.name)} className='w-full flex justify-between items-center text-lg text-left hover:text-blue-600 cursor-pointer'>
            {item.name}
            <FaChevronDown className='h-4 w-3 ml-2'/>
          </button>
